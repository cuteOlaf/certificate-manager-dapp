...
contract('CertificateRegistry', (accounts) => {
  ...
  const hash1 =
    '0x94f3e4c13989c51472ce78354b5205c5411f82e83c745b6f675e0c9aeb8ab4d1'
  const hash2 =
    '0x236d57dcd13712aa216d1f24317c32aaed9032bdd8fa83961a53a21ed95ded2d'
  let block
  ...
  describe('store', async () => {
    ...
    before(async () => {
      result = await certificateRegistryInstance.storeHash(hash1, {
        from: educator,
      })
      // console.log({result})
    ...
    it('should not store empty input', async () => {
      ...
      await certificateRegistryInstance.storeHash('', { from: educator }).should
        .be.rejected
    })

    it('should reject a hash if one is already stored for the supplied hash', async () => {
      await certificateRegistryInstance.storeHash(hash1, { from: educator })
        .should.be.rejected
    })

    it('should not store hash when user is not owner', async () => {
      await certificateRegistryInstance.storeHash(hash1, { from: student })
        .should.be.rejected
      await RegistryInstance.storeHash(hash1, { from: verifier }).should.be
        .rejected
    })
  })
  describe('verifyCertificateData', async () => {
    ...
    before(async () => {
      ...
      validHash = await certificateRegistryInstance.verifyCertificateData(
        hash1,
        actualBlockNumber,
      )
      inValidHash = await certificateRegistryInstance.verifyCertificateData(
        hash2,
        actualBlockNumber,
      )
      inValidBlockNumber = await certificateRegistryInstance.verifyCertificateData(
        hash1,
        blockNumber,
      )
    })
